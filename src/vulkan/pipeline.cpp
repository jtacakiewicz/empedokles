...
Pipeline::Pipeline(Device &device, const std::string &computeFilepath, const PipelineConfigInfo &configInfo)
    : m_device { device }
{
    m_layout = configInfo.pipelineLayout;
    createComputePipeline(computeFilepath, configInfo);
}
void Pipeline::createComputePipeline(const std::string &computeFilepath, const PipelineConfigInfo &configInfo)
{
    ...
    if(vkCreateComputePipelines(m_device.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline)) {
                                      ...
                                      const PipelineConfigInfo &configInfo)
{
    ...
    //  for optimizing performance for deriving pipelines from already exsiting
    //  ones
...
void Pipeline::createShaderModule(const std::vector<char> &code, VkShaderModule *shaderModule)
{
    ...
    //  default vector alloc ensures memory alignment so cast is valid
...
void Pipeline::bindDescriptorSets(VkCommandBuffer command_buffer, VkDescriptorSet *sets, uint32_t first_set, uint32_t set_count,
                                  uint32_t dynamic_offset_count, uint32_t *pDynamic_offsets)
{
    VkPipelineBindPoint bind_point =
        ((m_compute_shader_module != VK_NULL_HANDLE) ? VK_PIPELINE_BIND_POINT_COMPUTE : VK_PIPELINE_BIND_POINT_GRAPHICS);
    vkCmdBindDescriptorSets(command_buffer, bind_point, m_layout,
                            first_set,  //  starting set (0 is the globalDescriptorSet)
                            set_count,  //  set count
                            sets, dynamic_offset_count, pDynamic_offsets
