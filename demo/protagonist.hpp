...
struct Protagonist {
    Entity id;
    std::vector<vec2f> shape = { vec2f(-100.f / 4, -100.f / 1.5), vec2f(-100.f / 4, 100.f / 1.5), vec2f(100.f / 4, 100.f / 1.5),
    ...
    void setupPhysics(Coordinator &ECS)
    {
        ...
        ECS.getSystem<ColliderSystem>()
            ...
            .onCollisionExit(id, id, [&](Entity me, Entity other) {
    ...
    void setupAnimation(Coordinator &ECS)
    {
        ...
        AnimatedSprite::Builder build("idle", idle_moving);
        {
            ...
            build.addEdge("idle", "run",
                          [this, &ECS](Entity owner) { return abs(ECS.getComponent<Rigidbody>(owner)->velocity.x) > 25.f; });
            build.addEdge("run", "idle", [&, this](Entity owner, bool isended) {
                return abs(ECS.getComponent<Rigidbody>(owner)->velocity.x) < 25.f;
            });
            auto isJumping = [&, this](Entity owner) { return ECS.getComponent<Rigidbody>(owner)->velocity.y < -100.f; };
            auto isFalling = [&, this](Entity owner) { return ECS.getComponent<Rigidbody>(owner)->velocity.y > 25.f; };
            auto hasFallen = [&](Entity owner) { return isGrounded != false; };
            auto isAttackingCh = [&](Entity owner) { return isAttacking; };
            auto isRollingCh = [&](Entity owner) { return isRolling; };
