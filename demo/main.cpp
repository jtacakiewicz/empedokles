...
enum CollisionLayers { GROUND, PLAYER, FRIENDLY, ITEM };
...
public:
    ...
    std::vector<vec2f> unit_cube = { vec2f(-1.f / 2.f, -1.f / 2.f), vec2f(-1.f / 2.f, 1.f / 2.f), vec2f(1.f / 2.f, 1.f / 2.f),
                                     vec2f(1.f / 2.f, -1.f / 2.f) };
    std::vector<vec2f> cube_model_shape = { vec2f(-cube_side_len / 2, -cube_side_len / 2),
...
void Demo::onSetup(Window &window, Device &device)
{
    ...
    for(int i = 0; i < 4; i++) {
        ...
        ECS.addComponent(platform, Transform(ops[i].first, ops[i].second, { getWidth() / cube_side_len * 2, 1.f }));
...
void Demo::onUpdate(const float delta_time, Window &window, KeyboardController &controller)
{
    ...
    ECS.getComponent<Transform>(mouse_entity)->position = controller.global_mouse_pos();
    if(ECS.isEntityAlive(protagonist.id)) {
        ...
        if(ECS.isEntityAlive(protagonist.id)) {
            ...
            ECS.getComponent<Rigidbody>(protagonist.id)->velocity.x +=
                direction * (protagonist.speed * 0.5 + protagonist.speed / 2.f * (protagonist.isGrounded != false)) * delta_time;
    ...
    if(controller.get(eKeyMappings::Ability1).pressed) {
        ...
        ECS.addComponent(entity, Transform(vec2f(controller.global_mouse_pos()), 0.f));
